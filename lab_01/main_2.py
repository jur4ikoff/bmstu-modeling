...
def exact_task1(x: float) -> float:
    """Точное аналитическое решение: cos(sqrt(x)) при x >= 0, cosh(sqrt(-x)) при x < 0"""
    if x >= 0.0:
...
def euler_task1_backward(x_min: float, step: float) -> list[tuple[float, float]]:
    """Метод Эйлера назад от x = -step до x_min (для области x < 0, где нет особенности)."""
    x0 = -step
    u = taylor_4(x0)
    # u'(x) approx -1/2 + x/12 - x^2/240
    u_deriv = -0.5 + x0 / 12.0 - x0 * x0 / 240.0

    results = [(x0, u)]
    x = x0

    while x - step >= x_min - 1e-12:
        du = u_deriv
        du_deriv = -(2.0 * u_deriv + u) / (4.0 * x)
...
def run_task1():
    ...
    # График: все кривые от -5 до 40
    x_plot_min = -10.0
    n_neg = int(-x_plot_min / step)
    xs_full = [x_plot_min + i * step for i in range(n_neg + n_points)]

    ys_taylor = [taylor_4(x) for x in xs_full]
    ys_exact = [exact_task1(x) for x in xs_full]

    # Эйлер: назад от ~0 до -5, затем вперёд от 0 до 40
    euler_bwd = euler_task1_backward(x_plot_min, step)
    euler_full = euler_bwd + euler_data
    xs_euler = [pt[0] for pt in euler_full]
    ys_euler = [pt[1] for pt in euler_full]

    # cosh(sqrt(-x)) -- отдельная кривая при x < 0
    xs_neg = [x for x in xs_full if x < 0.0]
    ys_cosh = [math.cosh(math.sqrt(-x)) for x in xs_neg]

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=xs_full, y=ys_exact, name="Точное решение"))
    fig.add_trace(go.Scatter(x=xs_full, y=ys_taylor, name="Ряд Тейлора (4 члена)"))
    fig.add_trace(go.Scatter(x=xs_euler, y=ys_euler, name="Метод Эйлера"))
    fig.add_trace(go.Scatter(x=xs_neg, y=ys_cosh, name="cosh(sqrt(-x))"))
