...
T_TABLE = np.array(
    [2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000], dtype=float
)
SIGMA_TABLE = np.array(
    [0.309e-3, 0.309e-2, 0.309e-1, 0.270, 2.05, 6.06, 12.0, 19.9, 29.6, 41.1, 54.1]
)
LAMBDA_TABLE = np.array(
    [
        0.381e-3,
        0.381e-3,
        0.381e-3,
...
T_K_TABLE = np.array(
    [2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000], dtype=float
)
K_TABLE = np.array(
    [
        8.200e-03,
...
T_0 = 8000.0
T_W = 1800.0
P = 7 # параметпр формы начального температурного профиля
I_MAX = 1000.0
t_MAX = 80e-6
...
def solve_radiation(r, T):
    """Расчет u(r) метод простых итераций"""
    N = len(r) - 1
    h = r[1] - r[0]
    R_max = r[-1]
...
def step_T(r, T, tau, I_now, T_w, max_iter=30, eps=1e-4, omega=0.7):
    """Вычисляет один шаг по времени T"""
    N = len(r) - 1
    h = r[1] - r[0]
...
def solve(N=100, tau=1e-7, t_end=200e-6, verbose=False):
    ...
    for n in range(n_steps):
        ...
        if verbose and (n + 1) % max(1, n_steps // 10) == 0:
            print(
                f"  t = {t_new * 1e6:7.2f} мкс,  T(0) = {T[0]:7.1f} К,  "
...
def plot_T_profiles(
    r, t_arr, T_hist, t_max_pulse, save_times=None, fname="lab4_profiles.png"
):
    """График"""
    if save_times is None:
        save_times = list(np.linspace(t_max_pulse * 0.1, t_max_pulse, 5)) + list(
            np.linspace(t_max_pulse * 1.2, t_arr[-1], 5)
        )
...
if __name__ == "__main__":
    ...
    # Основной расчёт
    r, t_arr, T_hist, I_hist, E_hist = solve(
        N=100, tau=1e-7, t_end=t_MAX, verbose=True
    )
