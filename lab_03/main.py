...
_k_v1   = np.array([8.200e-3,2.768e-2,6.560e-2,1.281e-1,
                    2.214e-1,3.516e-1,5.248e-1,7.472e-1,1.025e0])
_k_v2   = np.array([1.600e0,5.400e0,1.280e1,2.500e1,
                    4.320e1,6.860e1,1.024e2,1.458e2,2.000e2])
_lnT   = np.log(_T_data)
...
def _rk2(chi, variant, N_steps=1000):
    ...
    for _ in range(N_steps):
        ...
        r_arr.append(r); u_arr.append(y[0]); F_arr.append(y[1])
...
def _shoot(chi, variant, method='RK45', max_step_div=500):
    ...
    return solve_ivp(_ode, [1e-10, R], [u0, 0.0],
                     args=(variant,), method=method,
                     dense_output=True,
    ...
    if variant == 1:
        ...
        for i in range(len(vals) - 1):
            if vals[i] * vals[i+1] < 0:
                chi_star = brentq(_bc_res, chis[i], chis[i+1],
                                  args=(variant,), xtol=1e-10)
                return _shoot(chi_star, variant, max_step_div=1000), chi_star
        raise RuntimeError("Корень не найден для варианта 1")
    else:
        chi_star = brentq(_bc_res, 0.9999985, 0.9999995,
                          args=(variant, 'Radau', 50), xtol=1e-12)
...
def solve_shooting_rk2(variant, N_steps=1000):
    ...
    for i in range(len(vals) - 1):
        if vals[i] * vals[i+1] < 0:
            chi_star = brentq(_bc_res_rk2, chis[i], chis[i+1],
                              args=(variant, N_steps), xtol=1e-8)
...
def solve_shooting_rk4(variant, N_steps=1000):
    ...
    for i in range(len(vals) - 1):
        if vals[i] * vals[i+1] < 0:
            chi_star = brentq(_bc_res_rk4, chis[i], chis[i+1],
                              args=(variant, N_steps), xtol=1e-8)
...
def plot_task2():
    # ── График 1: основное решение (solve_ivp, только вариант 1) ──
    variant = 1
    ...
    ax.set_title(f'Задача 2, Вариант 1  (chi={chi:.7f})')
    ax.set_xlabel('r, см')
    ax.legend(); ax.grid(True)
    plt.tight_layout()
    plt.savefig('task2.png', dpi=150)
    plt.close()
    print(f"Задача 2, Вариант 1: chi* = {chi:.8f}")
    ...
    ax.set_xlabel('r, см'); ax.legend(); ax.grid(True)
    ...
    ax2.set_xlabel('r, см'); ax2.legend(); ax2.grid(True)
    ...
    u_ref = np.interp(sol_ref.t, sol_ref.t, sol_ref.y[0])
    ...
    ax.set_xlabel('Шаг h'); ax.set_ylabel('Максимальная погрешность')
    ...
    ax2.set_xlabel('Число шагов N'); ax2.set_ylabel('Максимальная погрешность')
    ...
    print("\nЗадача 2 — OK. Графики: task2.png, task2_rk_compare.png, task2_convergence.png")
...
def solve_fdm(variant, N=300):
    ...
    cumint    = np.concatenate([[0.0],
                    np.cumsum(0.5 * h * (integrand[:-1] + integrand[1:]))])
    F_int = np.zeros(N + 1)
...
def compare_tasks23():
    ...
    print(f"Вариант 1: u(R) стрельба={u_s[-1]:.4e}, "
